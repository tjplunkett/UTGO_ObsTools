...
seeing_df = pd.DataFrame(columns=['Seeing'], data = ['Good (1.5")', 'Average (2.5")', 'Poor (3.5")'])

# Create a dataframe with this information
night_dict = {'Quantity': ['Latitude (deg)', 'Longitude (deg)', 'Height (m)','Julian Day','RA (deg)', 'RA (hms)', 'DEC (deg)', 'DEC (dms)', 'l (deg)', 'b (deg)', 'Constellation', 'Ast. Twilight Ends', 'Ast. Morning Starts', 'Meridian Transit', 'Moon Phase (deg)', 'Perc. Illuminated', 'Moon Seperation (deg)'], 'Value': [-42.43, 147.3, 646, jd, 219.874,'14:39:29.72',-60.83,'-60:49:56.00', round(l,3), round(b,3), 'Centaurus', str(night_time.iso), str(morning_time.iso), str(meridian_transit.iso), round(np.rad2deg(m_phase.value),3), round(m_illum,3), str(round(moon_sep,3))]}
night_df = pd.DataFrame(data=night_dict)

# Load the figures
sky_chart = create_star_chart(str(default_date.date())+" 00:00", 12, 200, 219.874, -60.93, 0)
airmass = plot_airmass(str(219.874), str(-60.83), default_date) # Alpha cent by default...
snrvsexp = plot_SNRvsTime('14:39:29.72', '-60:49:56.00', 14.0, 'r', get_npix('Average (2.5")'), default_date)
magvstime = plot_MagLimvsTime('14:39:29.72', '-60:49:56.00', 10.0, 'r', get_npix('Average (2.5")'), default_date)
snrvsmag = plot_SNRvsMag('14:39:29.72', '-60:49:56.00', 30.0, 'r', get_npix('Average (2.5")'), default_date)
...
app.layout = html.Div([
    ...
    html.Div([html.Label('Please input the target RA (in deg or sexigesimal): ', style={'width':'10vw'}), dcc.Input(id = 'RA', value = '14:39:29.72', placeholder='14:39:29.72', type='text', style={'margin-right':'10px', 'width':'8vw'}),
    html.Label('Please input the target DEC (in deg or sexigesimal): ', style={'width':'10vw'}), dcc.Input(id = 'DEC', value='-60:49:56.00', placeholder='-60:49:56.00', type='text', style={'margin-right':'10px', 'width':'8vw'}),
    ...
    html.H2(children = 'Exposure Time Calculator', style = {'textAlign':'left'}), html.Label('Please select a telescope: ', style={'margin-right':'85px'}), html.Label('Please select a filter: ', style = {'margin-right':'115px'}), html.Label('What is the seeing like?', style = {'margin-right':'105px'}),
html.Div([dcc.Dropdown(tele_df['Telescopes'], '50cm', id='TeleSelector', style={'width':'200px', 'margin-right':'50px'}), dcc.Dropdown(filter_df['Filters'], 'r', id='FilterSelector', style={'width':'200px', 'margin-right':'50px'}), dcc.Dropdown(seeing_df['Seeing'], 'Average (2.5")', id='SeeingSelector', style={'width':'200px', 'margin-right':'50px'})], style = {'display':'flex'}), html.Br(),
    
html.Label('Time from midnight (hours): '), html.Div([dcc.Slider(-6, 6, step=1, value=0, id='TimeSlider')], style={'width':'98vw'}), html.Br(),
    
html.Div([html.Div([html.Label('Magnitude: '), dcc.Input(id='sve_mag', value = 14.0), dcc.Graph(id='snrvexp_fig', figure=snrvsexp, style = {'width':'32vw'})]), html.Div([html.Label('SNR: '), dcc.Input(id='mvt_snr', value = 10.0), dcc.Graph(id='magvstime_fig', figure=magvstime, style = {'width':'32vw'})]), html.Div([html.Label('Exposure (s): '), dcc.Input(id='svm_exp', value = 30.0), dcc.Graph(id='snrvsmag_fig', figure=snrvsmag, style = {'width':'32vw'})])], style = {'display':'flex'})
...
@app.callback(
    ...
    State('DEC', 'value'),
    State('ObsDate', 'date'))
def update_table(n_clicks, ra, dec, date_value):
    date_object = datetime.fromisoformat(date_value)
    if n_clicks is None:
        raise PreventUpdate
        
    # Recalculate values for the tables with new inputs
    ra_deg, dec_deg = convert2Deg(ra, dec)
    ra_sex, dec_sex = convert2Sex(ra, dec)
    ...
    # Put into dataframe
    night_dict = {'Quantity': ['Latitude (deg)', 'Longitude (deg)', 'Height (m)','Julian Day','RA (deg)', 'RA (hms)', 'DEC (deg)', 'DEC (dms)', 'l (deg)', 'b (deg)', 'Constellation', 'Ast. Twilight Ends', 'Ast. Morning Starts', 'Meridian Transit', 'Moon Phase (deg)', 'Perc. Illuminated', 'Moon Seperation (deg)'], 'Value': [-42.43, 147.3, 646, jd, np.round(ra_deg, 3), str(ra_sex), round(dec_deg, 3), str(dec_sex), round(l_new,3), np.round(b_new,3), constel, str(night_time_new.iso), str(morning_time_new.iso), str(meridian_transit.iso), round(np.rad2deg(m_phase.value),3), round(m_illum, 3), str(round(moon_sep, 3))]}
    night_df = pd.DataFrame(data=night_dict)
...
@app.callback(
    ...
    State('DEC', 'value'),
    State('ObsDate', 'date'))
def update_airmass(n_clicks, ra, dec, date_value):
    date_object = datetime.fromisoformat(date_value)
    if n_clicks is None:
        raise PreventUpdate
    ra_deg, dec_deg = convert2Deg(ra, dec)
...
@app.callback(
    ...
    State('DEC', 'value'),
    State('ObsDate', 'date'))
def update_skychart(n_clicks,ra, dec, date_value):
    date_object = datetime.fromisoformat(date_value)
    if n_clicks is None:
        raise PreventUpdate
    ra_deg, dec_deg = convert2Deg(ra, dec)
...
@app.callback(
    ...
    State('DEC', 'value'),
    State('ObsDate', 'date'))
def update_snrvsexp(fltr, mag, seeing, t_delta, ra, dec, date_value):
    date_object = datetime.fromisoformat(date_value) + timedelta(hours=t_delta)
    if mag == '' or ra == None or dec == None:
        raise PreventUpdate
    else:
        n_pix = get_npix(str(seeing))
        snrvsexp = plot_SNRvsTime(str(ra), str(dec), float(mag), fltr, n_pix, date_object)
...
@app.callback(
    ...
    State('DEC', 'value'),
    State('ObsDate', 'date'))
def update_snrvsexp(fltr, snr, seeing, t_delta, ra, dec, date_value):
    date_object = datetime.fromisoformat(date_value) + timedelta(hours=t_delta)
    if snr == '' or ra == None or dec == None:
        raise PreventUpdate
    else:
        n_pix = get_npix(str(seeing))
        magvstime = plot_MagLimvsTime(str(ra), str(dec), float(snr), fltr, n_pix, date_object)
...
@app.callback(
    ...
    State('DEC', 'value'),
    State('ObsDate', 'date'))
def update_snrvsexp(fltr, exp, seeing, t_delta, ra, dec, date_value):
    date_object = datetime.fromisoformat(date_value) + timedelta(hours=t_delta)
    if exp == '' or ra == None or dec == None:
        raise PreventUpdate
    else:
        n_pix = get_npix(str(seeing))
        snrvsmag = plot_SNRvsMag(str(ra), str(dec), float(exp), fltr, n_pix, date_object)
